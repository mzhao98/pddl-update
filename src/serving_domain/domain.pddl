(define (domain meal-planning)
    ...
    (:action allergy-query
        :parameters (?h - human ?rec - recipe)
        :precondition (recipe-selected ?rec)
        :effect (allergens-identified)
    )

    (:action resolve-allergies
        :parameters (?h - human ?rec - recipe)
        :precondition (allergens-identified)
        :effect (allergen-free ?rec)
    )
)